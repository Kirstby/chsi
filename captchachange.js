...
function VerifyImage(eleID, opt/*这个主要用来修改默认样式属性的*/){
	var initStyleObject = {'imageStylePattern':'cursor:pointer;margin-bottom:1px', 'changeToolStylePattern':'color:#666;cursor:pointer','changeToolText':'看不清楚？换一个（字母区分大小写）'};
	if(!opt){
	    opt = initStyleObject;
	}
	var c = verifyCounter.length;
	this.imageURISubfix = "{{_random_}}";
    this.imageURI =  "www.chsi.com.cn/cet/ValidatorIMG.JPG?ID=";
	...
	this.templete=('<img id="'+eleID+'" onclick="getVerify(\''+eleID+'\')" style="'+this.imageStylePattern+'" src="' + this.initImageURI +'" alt="验证码读取中..." title="点击更换"/><br /><span onclick="getVerify(\''+eleID+'\','+c+')" style="'+this.changeToolStylePattern+'">'+this.changeToolText+'</span>');
	this.seed = function(){
